...
- **Service**:
  - `FolderNameGenerator` — 입력값을 받아 생성할 폴더 이름 목록(문자열 리스트)을 계산하는 순수 로직. 파일 시스템에 접근하지 않는다 → 이름 규칙 상세는 [doc/03-folder-naming-spec.md](03-folder-naming-spec.md).
  - `FolderCreationService` — 상위 폴더 기준으로 각 이름이 이미 존재하는지 확인하고, 없는 것만 실제로 생성한다. 결과(생성됨/건너뜀 개수, 실패 항목)를 반환한다.
- **ViewModel**: `MainViewModel` — 입력 필드 바인딩, "미리보기"/"폴더 생성" 커맨드, 미리보기 목록(이름 + 상태: 신규/이미 존재) 노출.
